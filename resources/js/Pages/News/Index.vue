...
const activeCategory = ref(props.activeCategory || 'all'); // Kategori aktif, default 'all'
const searchQuery = ref(''); // Query pencarian

// Filter berita berdasarkan kategori
const filterByCategory = (category) => {
  activeCategory.value = category; // Set kategori aktif
...
const showAllNews = () => {
  activeCategory.value = 'all'; // Set kategori aktif ke 'all'
  newsItems.value = allNews.value;
};
...
const deleteNews = (id) => {
  if (confirm('Are you sure you want to delete this news?')) {
    // Menggunakan Inertia untuk menghapus data
    Inertia.delete(`/news/${id}`, {
      onSuccess: () => {
        // Setelah sukses menghapus, refresh data berita
...
// Jika ada kategori aktif saat komponen dimuat, filter berita berdasarkan kategori tersebut
onMounted(() => {
...
<template>
  <div class="min-h-screen bg-gray-100">
    ...
    <main class="container mx-auto mt-10 p-6 bg-white shadow rounded">
      <h1 class="text-2xl font-bold text-center mb-6">News List</h1>
